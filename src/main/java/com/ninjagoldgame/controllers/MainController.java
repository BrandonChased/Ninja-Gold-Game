...
public class MainController {
    ...
    public String index(
        ...
        if (farm != null) {
            ...
            gold += amount;
            activities.add(0,
                    "You entered a farm and earned " + amount + " gold. (" + simpleFormat.format(new Date()) + ")");
            session.setAttribute("activities", activities);
            session.setAttribute("gold", gold);
            return "redirect:/";
        }
        ...
        if (cave != null) {
            ...
            gold += amount;
            activities.add(0,
                    "You entered a cave and earned " + amount + " gold. (" + simpleFormat.format(new Date()) + ")");
            session.setAttribute("activities", activities);
            session.setAttribute("gold", gold);
            return "redirect:/";
        }
        ...
        if (quest != null) {
            ...
            gold += amount;
            if (amount >= 0) {
                activities.add(0, "You completed a quest and earned " + amount + " gold. ("
                        + simpleFormat.format(new Date()) + ")");
            } else {
                activities.add(0, "You failed a quest and lost " + (amount * -1) + " gold. ("
                        + simpleFormat.format(new Date()) + ")");
            }
            session.setAttribute("activities", activities);
            session.setAttribute("gold", gold);
